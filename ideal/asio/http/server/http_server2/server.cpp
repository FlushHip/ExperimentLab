...
HttpServer::HttpServer(std::string_view address, std::string_view port, std::string_view doc_root)
    : io_context_pool_()
    , acceptor_(io_context_pool_.io_context())
    , signals_(io_context_pool_.io_context())
    , request_handler_(doc_root)
{
    ...
    boost::asio::ip::tcp::resolver resolver(io_context_pool_.io_context());
...
{
    auto new_connection = std::make_shared<Connection>(io_context_pool_.io_context()
