#include "headers.h"

struct ListNode {
    int val;
    ListNode* next;
    ListNode() : val(0), next(nullptr) {}
    ListNode(int x) : val(x), next(nullptr) {}
    ListNode(int x, ListNode* next) : val(x), next(next) {}

    friend std::ostream& operator<<(std::ostream& os, const ListNode* ptr) {
        os << "[";
        for (; ptr != nullptr; ptr = ptr->next) {
            os << ptr->val;
            if (ptr->next != nullptr) {
                os << ", ";
            }
        }
        os << "]";
        return os;
    }
};

class Solution {
public:
    ListNode* mergeKLists(vector<ListNode*>& lists) {
        return lists.empty() ? nullptr : merge_listk(lists);
    }

private:
    ListNode* merge_listk(std::vector<ListNode*>& lists) {
        std::partition(lists.begin(), lists.end(),
            [](const ListNode* ptr) { return ptr != nullptr; });
        lists.erase(std::partition(lists.begin(), lists.end(),
                        [](const ListNode* ptr) { return ptr != nullptr; }),
            lists.end());
        auto cmp = [](const ListNode* lhs, const ListNode* rhs) {
            return lhs->val > rhs->val;
        };
        std::priority_queue<ListNode*, std::vector<ListNode*>, decltype(cmp)>
            que(cmp, lists);
        ListNode* root = nullptr;
        for (ListNode* q = nullptr; !que.empty();) {
            ListNode* ptr = que.top();
            que.pop();
            if (root == nullptr) {
                q = root = ptr;
            } else {
                q = q->next = ptr;
            }
            if (ptr->next != nullptr) {
                que.push(ptr->next);
            }
        }
        return root;
    }

    ListNode* merge_list2(ListNode* lhs, ListNode* rhs) {
        if (lhs == nullptr || rhs == nullptr) {
            return lhs == nullptr ? rhs : lhs;
        }
        ListNode *root = lhs->val < rhs->val ? lhs : rhs, *ptr = root;
        for (root == lhs ? (lhs = lhs->next) : (rhs = rhs->next);
             lhs != nullptr && rhs != nullptr; ptr = ptr->next) {
            if (lhs->val < rhs->val) {
                ptr->next = lhs;
                lhs = lhs->next;
            } else {
                ptr->next = rhs;
                rhs = rhs->next;
            }
        }
        if (lhs == nullptr) {
            ptr->next = rhs;
        } else {
            ptr->next = lhs;
        }
        return root;
    }
};
